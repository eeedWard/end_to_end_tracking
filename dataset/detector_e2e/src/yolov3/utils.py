...
def clip_coords_single_box(boxes, img_shape):
    # Clip bounding xyxy bounding boxes to image shape (height, width)
    boxes[0] = bound(0, img_shape[1], boxes[0])  # clip x
    boxes[2] = bound(0, img_shape[1], boxes[2])  # clip x
    boxes[1] = bound(0, img_shape[0], boxes[1])  # clip y
    boxes[3] = bound(0, img_shape[0], boxes[3])  # clip y
    return boxes
...
def non_max_suppression(prediction, classes_selected_id_list, conf_thres=0.5, nms_thres=0.5, device="cpu"):
    """
    ...
    for image_i, pred in enumerate(prediction):
        ...
        # merge truck(7), bus(5) into car(2)
        classes_to_merge = torch.LongTensor([5, 7]).to(device)
        class_pred[(class_pred.view(-1,1) == classes_to_merge.view(1,-1)).any(1)] = 2

        # Remove classes (optional)
        pred_copy = pred[:, 4].clone()
        pred[:, 4] = 0.0 #set confidence to 0, then replace with the original value for selected classes only
        for class_id in classes_selected_id_list:
            pred[class_pred == class_id, 4] = pred_copy[class_pred == class_id]
